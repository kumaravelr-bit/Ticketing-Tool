...
export default function OfferLetterEdit() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await getOfferLetterById(id);
...
const handleChange = ({ target: { name, value } }) => {
  setIsChanged(true); // ✅ mark changed
...
const handleUpdate = async () => {
  try {
    ...
    await updateOfferLetter(id, formData);
  ...
  return (
    <div className={styles.container}>
      ...
      <h3>Editing: {formData.document_id}</h3>
