...
export default function EditPayslip() {
    ...
    const loadPayslip = async () => {
      try {
        ...
        setFormData({
          ...
          salary_date: row.salary_date ? String(row.salary_date).slice(0, 10) : "",
          account_number: row.account_number || "",
          lop: safeNumber(row.lop),
          salary_days: safeNumber(row.salary_days) + safeNumber(row.lop),
          remarks: row.remarks || "",
          components: mapComponentsFromApi(row.components),
        });

        setEmployeeSearch(`${row.emp_id || ""} - ${row.employee_name || ""}`.trim());

        const asyncLoads = [];
        if (row.zone_id) {
          asyncLoads.push(
            getBranchesByZone(row.zone_id).then((branchRes) => setBranches(branchRes.data || [])),
          );
        }
        if (row.team_id) {
          asyncLoads.push(
            getDesignations(row.team_id).then((designationRes) => setDesignations(designationRes.data || [])),
          );
        }
  ...
  const handleEmployeeSelect = async (employee) => {
    setFormData((prev) => ({
      ...
      account_number: employee.account_number || prev.account_number,
  ...
  if (loading) {
    return (
      <div className={styles.container}>
        <h2 className={styles.title}>Loading Payslip...</h2>
      </div>
    );
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <h2 className={styles.title}>Edit Payslip</h2>
      ...
      <div className={styles.grid}>
        ...
        <div className={styles.full}>
          <button className={styles.button} onClick={handleSubmit} disabled={submitting}>
            {submitting ? "Updating..." : "Update Payslip"}
          </button>
        </div>
      </div>
    </div>
